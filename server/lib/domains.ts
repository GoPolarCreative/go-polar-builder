 ...
 *     runs RDAP for .au.
 *   - DNS over HTTPS for MX, NS and A records, from Google Public DNS by default.
 *   - WHOIS on port 43 over a plain TCP socket when RDAP has nothing, which is still the case
 *     for some ccTLDs.
 *
...
// Google Public DNS. Overridable with DOH_ENDPOINT if a different resolver is preferred.
const DOH = process.env.DOH_ENDPOINT ?? 'https://dns.google/resolve'
...
`))
    socket.on('data', (chunk: Buffer) => {
      ...
      if (total > 64_000) done(Buffer.concat(chunks).toString('utf8'))
    })
...
export async function inspectDomain(input: string): Promise<DomainReport> {
  ...
  // Demo mode never touches a registry. The shape is identical so the screen is reviewable.
  if (domain && config().demoMode) return fakeDomainReport(domain)
...
export async function checkAvailability(
  ...
  if (config().demoMode) {
    const fake = fakeAvailability(domain)
    return { ...fake, detail: fake.detail }
  }
