...
1b. THE SECTION LABELS ARE YOURS TO WRITE, AND THEIRS TO CHANGE. sectionCopy carries the small label above each section heading, plus the heading and blurb on the sections where the template supplies one. Leave a section out and the built-in wording is used, which is the right answer most of the time. Fill one in when the trade has better words for it: a landscaper does not have to say "What we do".

It matters most on an EDIT. These labels used to live in the renderer, so a customer asking to change the words above a heading was asking for something nothing could do: the change reported success, cost them a round, and the label never moved. If somebody asks about a label, a section heading or the line under it, sectionCopy is where that change goes.
