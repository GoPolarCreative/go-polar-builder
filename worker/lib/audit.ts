 ...
 * inline prompt plus a decision the generator acts on, because a tradie who hits a hard error
 * at the end of a five step form does not come back.
 *
 * The image judgements lean on stats computed in the browser at upload time (src/lib/image.ts).
 * Workers have no image decoder, so measuring pixels server side would mean shipping a wasm
 * codec for a decision that a canvas already answers well.
 */
...
export function isUsablePhoto(a: AssetRecord): boolean {
  ...
  if ((a.bytes ?? 0) < 20_000) return false
  return true
}

/**
 * A "mockup render" is a logo composited onto a photograph: a sign, a van, a business card
 * shot. It cannot be used as a header logo because it carries its own background.
...
function looksLikeMockupRender(logo: AssetRecord): boolean {
  const s = logo.stats
  if (!s) return false
  if ((logo.content_type ?? '').includes('svg')) return false
...
export function runGapAudit(
  ...
  const logo = assets.find((a) => a.kind === 'logo' && a.id === intake.logoAssetId)
  ...
  if (!logo) {
    ...
    if (looksLikeMockupRender(logo)) {
      ...
      if (w > 0 && w < LOGO_MIN_WIDTH && !(logo.content_type ?? '').includes('svg')) {
        flags.push({
