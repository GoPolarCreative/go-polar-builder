...
Logo: ${facts.logoPath ? `supplied at ${facts.logoPath}` : 'none usable'}
Logo treatment you must use: ${logoTreatmentFor(auditFlags, Boolean(facts.logoPath))}
...
Logo file path: ${facts.logoPath ?? 'none, use the CSS logotype treatment'}
Photo paths, use these exact strings and no others:
${
  facts.photoPaths.length === 0
    ? '  none supplied. Build every image area with CSS gradients and leave CLIENT TO SUPPLY comments.'
    : facts.photoPaths.map((p) => `  ${p.path}  (assetId ${p.assetId})`).join('\n')
}
